...
function localBusiness(ctx: PageSchemaContext) {
  ...
  return {
    ...
    email: safe(brand.email),
    image: safe(brand.logoUrl),
    logo: safe(brand.logoUrl),
    priceRange: "$$",
    // SAB-safe: street omitted when empty (service-area business, city-level NAP)
    address: {
      "@type": "PostalAddress",
      streetAddress: safe(brand.streetAddress),
      addressLocality: brand.primaryCity,
      addressRegion: brand.primaryState,
      postalCode: brand.postalCode,
      addressCountry: "US",
    },
    geo: safe(brand.lat) && safe(brand.lng)
      ? {
          "@type": "GeoCoordinates",
          latitude: brand.lat,
          longitude: brand.lng,
    ...
    foundingDate: safe(brand.foundedYear),
...
function organization() {
  return {
    ...
    alternateName: brand.shortName,
    url: brand.canonicalUrl,
    logo: safe(brand.logoUrl) ? { "@type": "ImageObject", url: brand.logoUrl } : undefined,
    telephone: brand.phoneRaw,
    email: safe(brand.email),
    foundingDate: safe(brand.foundedYear),
    sameAs: brand.sameAsUrls,
